function test_simulation = build_test_simulation_circular( ...
    q0, ...
    omega0_deg_s, ...
    radius, ...
    ic_frame ...
...
thrusters = Thruster.empty();
for i=1:4
    thrusters(i) = Thruster(r(i,:),e(i,:));
end
...
simulationConfig.ControllerType = "sliding";
...
uoff=0.6*uon;
umax=1;
pwpf = PWPF(km,tau,uon,uoff,umax,kp,delta_t);

test_simulation = Simulation( ...
    spacecraft,bodyNames,mekf,pwpf,t0,tspan,simulationConfig);
end
